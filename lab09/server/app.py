...
async def respond_to_message(websocket, message):
    ...
    ********************************************************************/
    ...
    if data.get('type') == 'login':
        logged_in_users[websocket] = data.get('username')
        response['type'] = 'login'
        response['user_joined'] = data.get('username')
        response['active_users'] = list(logged_in_users.values())
    elif data.get('type') == 'disconnect':
        del logged_in_users[websocket]
        response['type'] = 'disconnect'
        response['user_left'] = data.get('username')
        response['active_users'] = list(logged_in_users.values())
    elif data.get('type') == 'chat':
        response = data
    else:
        response['Unrecognized message type'] = data
